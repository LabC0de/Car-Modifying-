...
coinImg = [pygame.image.load('coins_01.png'), pygame.image.load('coins_02.png'),
           pygame.image.load('coins_03.png'), pygame.image.load('coins_04.png'),
           pygame.image.load('coins_05.png'), pygame.image.load('coins_06.png')]
...
# the background strip but I'm planning to just put an image and not to fill only gray. I'm still working on it
def background():
    gamedisplays.blit(backgroundpic, (0, 0))
    gamedisplays.blit(backgroundpic, (0, 200))
    gamedisplays.blit(backgroundpic, (0, 400))
...
def game_loop():
    ...
    while not bumped:
        ...
        if y < coinsx+obs_height:
            # why obs_width -> is the coin a object of type obiscle? or does it has the same width
            if coinsx < x < coinsx + obs_width or x+car_width > coinsx and x+car_width < coinsx+obs_width:
                score += 2
...
# actually this is the hardest part of it i don't understand this code but what i understand it is the restriction
        if y < obs_starty+obs_height:
            """
            This is a colision detection based on simple squares
            
            x is your position -> so depending on x you crash if the square surrounding somehow intersects with
            the suare describing your car. (same goes for y) basically if the obs is at the heigth of your car
        ...
        obs_x < x < obs_x+obs_width
            
                x-----+ <- position x describing the car
                |     |
                | car |
                |     |
                +-----+
            
            Problem with "bounding box" collision is that its unprecise for eg tilted objects
            
            also the way the collision is checked here it would not trigger for a very small obstacle exactly in the 
            middle
            ...
            does not trigger colision
            
            better way to do this would be
            
            if x < obs_x < x + car_width or x < obs_x + obs_width < x + car_width 
            
            this checks if the obs is anywhere in between the cars boundarys 
